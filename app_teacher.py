...
# ==============================================================================
    ...
    # =========================================================================
    # 👑 本地多考对比分析 (🚀 纯依靠分数硬核动态算榜)
    # =========================================================================
    if menu_sel == "本地多考对比":
        st.info("💡 请上传包含各次考试独立Sheet的Excel文件。系统将无视原表自带的排名，根据实际成绩为您自动硬核算榜！")
        ...
        if uploaded_file:
            ...
            with st.container(border=True):
                ...
                # 🛡️ 删除了寻找 rank_kws 的逻辑，系统现在只抓成绩，其余全部自己算！
                ...
                if not score_col_a or not score_col_b:
                    ...
                    if '姓名' not in clean_a.columns or '姓名' not in clean_b.columns:
                        ...
                        if merged_df.empty:
                            ...
                            # 🚀 核心算榜引擎发动！(同分同名次，method='min')
                            merged_df[base_rank_col] = merged_df[base_score_col].rank(ascending=False, method='min').fillna(0).astype(int)
                            merged_df[comp_rank_col] = merged_df[comp_score_col].rank(ascending=False, method='min').fillna(0).astype(int)
                            
                            merged_df['分数进步'] = (merged_df[comp_score_col] - merged_df[base_score_col]).round(1)
                            # 排名进步：原排名(大) - 现排名(小) = 进步为正数
                            merged_df['排名进步'] = merged_df[base_rank_col] - merged_df[comp_rank_col]
                            
                            disp_cols = ['班级', '姓名', base_score_col, base_rank_col, comp_score_col, comp_rank_col, '分数进步', '排名进步']
                            ...
                            with c_table:
                                st.markdown("<p style='font-size:14px; color:#64748B;'>📝 学生比对明细 (黄色背景为进步生，校排名已由系统智能重算)</p>", unsafe_allow_html=True)
