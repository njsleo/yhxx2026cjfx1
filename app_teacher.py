...
# ==============================================================================
    ...
    # =========================================================================
    # 👑 专属特权模块：本地多考对比分析 (完全脱离固定配置，直接读取 Excel)
    # =========================================================================
    if menu_sel == "本地多考对比":
        st.info("💡 请上传由大型联考判卷系统导出的多 Sheet 成绩表（如同时包含段考和一模成绩的 Excel），系统将自动解析表头并进行进退步追踪计算。")
        
        uploaded_file = st.file_uploader("📥 点击或拖拽上传判卷系统 Excel 成绩表 (.xlsx / .xls)", type=["xlsx", "xls"])
